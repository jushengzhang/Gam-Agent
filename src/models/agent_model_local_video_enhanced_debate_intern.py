...
def load_video(video_path, bound=None, input_size=448, max_num=1, num_segments=32, get_frame_by_duration=False):
    ...
    logging.info(
        f"将从视频 {video_path} 加载 {len(frame_indices)} 帧 (索引: {frame_indices[:5]}...{frame_indices[-5:]})")
...
class AgentModel:
    ...
    def __init__(self, config):
        ...
        self.num_segments = config.get("num_segments", 32)  # Default from snippet
...
class ExpertAgentModel:
    ...
    def run_enhanced_debate_process(self, pixel_values, num_patches_list, instruction: str, options,
        ...
        for i, expert in enumerate(self.experts):
            ...
            expert_result = self.base_model.generate_response(pixel_values, num_patches_list, evidence_analysis_prompt, history=None)
        ...
        if uncertain_claims and self.critic_experts:
            ...
            for critic in self.critic_experts:
                ...
                critique_result = self.base_model.generate_response(pixel_values, num_patches_list, custom_critique_prompt, history=None)
            ...
            final_result = self.base_model.generate_response(pixel_values, num_patches_list, integration_prompt, history=None)
    ...
    def process_with_experts(self, video_path: str = None, instruction: str = "", output=None, options=None,
        ...
        try:
            ...
            return {
                ...
                "agent_uncertainties": [1.0] * len(self.experts) if self.experts else [1.0],
                # Handle empty experts list
                "agent_responses": [{"role": exp.get('name', f'expert_{i}'), "response": "[视频处理错误]"} for i, exp in
                                    enumerate(self.experts)] if self.experts else [],
        ...
        for i, expert in enumerate(self.experts):
            ...
            expert_result = self.base_model.generate_response(pixel_values, num_patches_list, expert_prompt, history=None)
        ...
        integration_result = self.base_model.generate_response(pixel_values, num_patches_list, integration_prompt, history=None)
