...
class ExpertAgentModel:
    ...
    def _parse_claims_and_confidence(self, analysis_text):
        ...
        for claim_match in claim_matches:
            ...
            # 在当前声明之后查找对应的置信度
            confidence_match = re.search(confidence_pattern, analysis_text[claim_end_pos:claim_end_pos+200], re.IGNORECASE)
            confidence = int(confidence_match.group(1)) if confidence_match else None
            
            # 查找证据描述
            evidence_match = re.search(evidence_pattern, analysis_text[claim_end_pos:claim_end_pos+500], re.IGNORECASE)
        ...
        if not claims:
            ...
            confidence_lines = re.finditer(r"([^.!?]*(?:confidence|确信度|信心)[^.!?]*[.!?])", analysis_text, re.IGNORECASE)
    ...
    def run_enhanced_debate_process(self, image_inputs: list[Image.Image], instruction: str, options, current_response: str, previous_critiques=None, reference_output=None, is_final_round=False):
        """
    ...
    def process_with_experts(self, image_inputs: list[Image.Image], instruction: str, output=None, options=None, choice_answer=None):
        ...
        for i, expert in enumerate(self.experts):
            ...
            self.logger.info(f"专家 '{expert['display_name']}' 完成初始分析, 不确定性: {expert_uncertainty:.4f}, 耗时: {expert_end_time - expert_start_time:.2f}s")
