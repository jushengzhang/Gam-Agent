...
class AgentModel:
    ...
    def generate_response(self, image_inputs: list[Image.Image], prompt: str):
        ...
        # 构造对话消息列表，按照 Qwen2.5-VL 的模板要求：消息中依次插入图片和文本内容
        if image_inputs:
            content = []
            for img in image_inputs:
                # 将每个 PIL image 转为 base64 编码的字符串
                img_b64 = self.pil_image_to_base64(img)
        ...
        if image_inputs:
            self.logger.info(f"使用 Qwen2.5-VL 模型生成响应，图像数量: {len(image_inputs)}，提示: '{prompt[:100]}...'")
        else:
            self.logger.info(f"使用 Qwen2.5-VL 模型生成响应，无图 (纯文本)，提示: '{prompt[:100]}...'")

        # 根据官方推荐，先将消息应用到模板中
        text_template = self.processor.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)

        # 从 qwen_vl_utils 导入 process_vision_info，提取图像（与视频）输入信息
        try:
        ...
        # 推理：调用 model.generate，设置 max_new_tokens 根据需要调整
        ...
        # 根据提示截掉输入部分 token（生成部分在后面），这里假设每个 batch 只有一条对话
...
class ExpertAgentModel:
    ...
    def _parse_claims_and_confidence(self, analysis_text):
        ...
        if not claims:
            ...
            for match in confidence_lines:
                ...
                confidence = int(conf_match.group(1)) if conf_match else 50  # 默认50%
                claims.append({
                    'claim': line,
                    'confidence': confidence,
                    'evidence': "",
    ...
    def run_enhanced_debate_process(self, image_inputs: list[Image.Image], instruction: str, options,
        ...
        Returns:
            dict: {'text': str, 'critiques': list, 'evidence_regions': list} 包含最终回答、批评和证据区域
        """
        self.logger.info("进入增强辩论流程（不确定性驱动议题筛选）...")

        # 1. 要求每个专家对当前回答进行分析，并标注证据区域
        evidence_analysis_prompt = (
            f"You are analyzing an image-based response. The original question was: {instruction}\n\n"
            ...
            f"   d) If possible, describe the image region (approximate x,y coordinates or description) where this evidence appears\n\n"
            f"Format your response as:\n"
            f"CLAIM 1: [claim text]\n"
            f"CONFIDENCE: [0-100%]\n"
            f"EVIDENCE: [description of visual evidence]\n"
            f"REGION: [description of image region]\n\n"
            f"CLAIM 2: ..."
        )
        ...
        for i, expert in enumerate(self.experts):
            ...
            expert_result = self.base_model.generate_response(image_inputs, evidence_analysis_prompt)
        ...
        # 2. 根据不确定性，筛选需要批评的议题（最不确定的Top-K）
        k = min(3, len(claim_confidences))  # 最多选择3个议题
        ...
        if uncertain_claims and self.critic_experts:
            ...
            integration_prompt_parts.append(
                ...
                "4. Synthesize a final answer that is accurate, complete, and addresses the original question.\n"
                "5. If options were provided, your final answer MUST select one of the options clearly.\n"
            )
            if is_final_round:  # 新增：如果是最后一轮则增加仅回复选项字母的描述
                integration_prompt_parts.append("6. Answer with the option's letter from the given choices directly.\n")
            ...
            final_result = self.base_model.generate_response(image_inputs, integration_prompt)
            ...
            # 如果没有不确定议题或批评专家，直接返回当前回答
            self.logger.info("无可用批评专家或不确定议题，保持当前回答")
            return {"text": current_response, "critiques": [], "evidence_regions": []}

    # 原有的辩论流程 (保留以备参考)
    # def run_debate_process_new(self, image_inputs: list[Image.Image], instruction: str, options, current_response: str, previous_critiques=None, reference_output=None):
    #     """
    ...
    #     return {"text": final_response, "critiques": []}

    def process_with_experts(self, image_inputs: list[Image.Image], instruction: str, output=None, options=None,
                             choice_answer=None):
        """
        使用专家处理图像和指令 (增强本地模式)
        ...
        for i, expert in enumerate(self.experts):
            ...
            # 添加证据区域提示
            expert_prompt += "\n\nImportant: For key claims in your analysis, please mention which parts of the image support your conclusions. When possible, describe the specific regions or features that provide evidence."
        ...
        # 2. 初始整合所有专家回答
        integration_prompt_parts = [
            "You are a Judge tasked with integrating analyses from multiple specialists who have examined the same image(s).",
            f"Original Question: {instruction}"
        ]
        ...
        integration_start_time = time.time()
        integration_result = self.base_model.generate_response(image_inputs, integration_prompt)
        ...
        if self.experts_settings.get('use_debate', True) and self.critic_experts:
            ...
            for i in range(debate_rounds):
                ...
                debate_output = self.run_enhanced_debate_process(
                    ...
                    is_final_round=(i == debate_rounds - 1)  # 新增：传递 is_final_round
                )
